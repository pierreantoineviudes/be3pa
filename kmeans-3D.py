...
def centre_de_nuage(Lst) :
    x_m =sum([x for (x,*_) in Lst])/len(Lst)
    y_m =sum([y for (_,y,_) in Lst])/len(Lst)
    z_m =sum([z for (*_, z) in Lst])/len(Lst)
    return (x_m, y_m, z_m)
#---------------------------------------------------------
def K_means_2_groupes_AVEC_option_plots_intermediaires(Lst, k=2, avec_plot_3D=True) :
    m_1 = Lst[0]
    ...
    while not  fini :
        ...
        if avec_plot_3D :    
            ...
            x1 = [x for (x,*_) in L_1]
            x2 = [x for (x,*_) in L_2]
            y1 = [y for (_,y,*_) in L_1]
            y2 = [y for (_,y,*_) in L_2]
            z1 = [z for (*_,z) in L_1]
            z2 = [z for (*_,z) in L_2]
            ...
            #ax.scatter(m_1[0], m_1[1], m_1[2], 'go') # add one point   on the figure
            ax.plot(m_1[0], m_1[1], m_1[2], 'ro')

            # Creating plot
            sctt = ax.scatter3D(x2, y2, z2,
                                alpha = 0.8,
                                c = z2, # (x1 + y1 + z1),
                                cmap = my_cmap,
                                marker ='+')
            ...
            #input(" ? ")
            iteration+=1
        elif False : # La version plot 2D (je ne le fais pas ici)
            ...
            plt.plot([x for (x,*_) in L_1], [y for (_,y,*_) in L_1], 'r+')
            plt.plot(m_1[0], m_1[1], 'go')
            #plt.axis([0, 6, 0, 20])
            #plt.show()
            plt.plot([x for (x,*_) in L_2], [y for (_,y,*_) in L_2], 'b+')
...
if __name__ == "__main__" :
    ...
    Lst1 = [(random.randint(1,2*Max//3), random.randint(1,2*Max//3), random.randint(1,2*Max//3)) for i in range(NB_points//2)]
    Lst2 = [(random.randint(Max//3, 3*Max//4), random.randint(Max//3, 3*Max//4), random.randint(Max//3, 3*Max//4)) for i in range(NB_points//2)]
    Lst=Lst1 + Lst2

    # Lst = [(random.randint(1,Max), random.randint(1,Max)) for i in range(NB_points)]
    # se limiter à [(0,0) .. (100,100)]
    m_1, L_1, m_2, L_2 = K_means_2_groupes_AVEC_option_plots_intermediaires(Lst)
    #print("Resultats : cluster 1 : ", (m_1, L_1), "cluster 2 : ", (m_2, L_2 ))
    print(f"Resultats (3D) : centre 1 : {m_1}  avec {len(L_1)} élés et centre 2 : {m_2} avec  {len(L_2)} élés")
    ...
    for i in range(NB_essais) :
        ...
        m_1, L_1, m_2, L_2 = K_means_2_groupes_AVEC_option_plots_intermediaires(Lst, avec_plot_3D=False)# Pour ne pas refaire les plots 
